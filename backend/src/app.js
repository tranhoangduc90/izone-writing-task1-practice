...
export function createApp({config,pool,service,lessonService=service,provisionalService=null,lmsResultService=null,teacherCommentService=null,teacherClassAccess=null,writingFlowService=null,writingFlowWebIntake=null,writingFlowWebQueue=null,writingFlowWebPortal=null,writingFlowStage=null,writingFlowHandoff=null,writingFlowAiCall=null,writingFlowScan=null,writingFlowTrccRepair=null,teacherAuth=null,adminAuth=(_q,r)=>r.status(503).json({ok:false,error:'ADMIN_AUTH_NOT_CONFIGURED'})}){
 ...
 const webPortalReady=(q,r,next)=>writingFlowWebPortal?next():r.status(503).json({ok:false,error:'WEB_PORTAL_NOT_READY'});
 ...
 const webPortal=(q,r,next)=>sameSecret((q.get('authorization')||'').replace(/^Bearer\s+/i,''),config.webSubstitutePortalToken)?next():r.status(401).json({ok:false,error:'UNAUTHORIZED'});
 ...
 // Bộ đồng bộ Portal dùng khóa riêng và chỉ mở khi cờ của nguồn mới được bật.
 app.post('/api/v1/internal/writing-flow/web-substitute/portal/claim',webPortal,
   webPortalReady,asyncRoute(async(q,r)=>{
     const {limit}=parse(z.object({limit:z.number().int().min(1).max(4).default(1)}),q.body);
     r.json({ok:true,jobs:await writingFlowWebPortal.claimDue({limit})});
   }));
 app.post('/api/v1/internal/writing-flow/web-substitute/portal/complete',webPortal,
   webPortalReady,asyncRoute(async(q,r)=>{
     const input=parse(z.object({submissionId:uuid,leaseToken:uuid,
       result:z.unknown()}),q.body);
     r.json({ok:true,receipt:await writingFlowWebPortal.completeSync(input)});
   }));
 app.post('/api/v1/internal/writing-flow/web-substitute/portal/review',webPortal,
   webPortalReady,asyncRoute(async(q,r)=>{
     const input=parse(z.object({submissionId:uuid,leaseToken:uuid,
       errorCode:z.enum(['WEB_PORTAL_PREVIEW_BLOCKED','WEB_PORTAL_WRITE_UNKNOWN',
         'WEB_PORTAL_READBACK_FAILED'])}),q.body);
     r.json({ok:true,receipt:await writingFlowWebPortal.markForReview(input)});
   }));
 app.post('/api/v1/internal/writing-flow/web-substitute/portal/expire',webPortal,
   webPortalReady,asyncRoute(async(q,r)=>{
     const {limit}=parse(z.object({limit:z.number().int().min(1).max(100).default(100)}),q.body);
     r.json({ok:true,summary:await writingFlowWebPortal.markExpiredForReview({limit})});
   }));
