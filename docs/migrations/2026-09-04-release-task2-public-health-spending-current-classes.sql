-- Dữ liệu nhận vào: activity ngân sách y tế ở trạng thái bản nháp, manifest đúng checksum,
-- hai lớp Task 2 hiện hành và các ngoại lệ roster đã được duyệt ở đề Living Alone.
-- Việc chính: mở đúng CS.070626 và CS.160826 đến hết ngày 31/12/2026,
-- sao chép ngoại lệ đã duyệt rồi dựng roster từ mapping/Classroom hiện hành.
...
BEGIN
  ...
  IF v_activity_count <> 1
    ...
    RAISE EXCEPTION
      'Phát hành đề ngân sách y tế chưa đạt: activity=%, sections=%, definition diff=%, scopes=%, class diff=%, roster diff=%, scope rỗng=%, alias trùng=%, provisional=%.',
      v_activity_count,
      v_section_count,
