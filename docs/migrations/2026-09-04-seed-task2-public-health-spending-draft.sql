-- Dữ liệu nhận vào: manifest đề chi ngân sách cho phòng bệnh hay chữa bệnh đã qua validator
-- và activity mẫu Task 2 đang ghim đúng bộ prompt dùng chung.
-- Việc chính: tạo phiên bản activity mới ở trạng thái bản nháp và sao chép đúng bốn
-- định nghĩa section từ template; chưa gắn lớp hoặc roster thật.
...
VALUES(
  '46f35f92-2d63-4389-b80d-51c53c6d959a',
  'writing-task2-public-health-spending',
  '2026-09-04.1',
  '578478d5fd4797e4ba6a1d260b209d578a60758a05877407cbecee0e520fb45b',
  'Luyện Writing Task 2: Phòng bệnh hay chữa bệnh',
  'It is more important to spend public money promoting a healthy lifestyle to prevent illness than to spend it on treating people who are already ill. To what extent do you agree or disagree?',
...
WITH target_activity AS (
  SELECT id
  FROM writing_practice.activity
  WHERE slug = 'writing-task2-public-health-spending'
...
BEGIN
  ...
  WHERE slug = 'writing-task2-public-health-spending'
    AND content_version = '2026-09-04.1'
    AND public_id = '46f35f92-2d63-4389-b80d-51c53c6d959a'
    AND manifest_checksum = '578478d5fd4797e4ba6a1d260b209d578a60758a05877407cbecee0e520fb45b'
    AND task_prompt = 'It is more important to spend public money promoting a healthy lifestyle to prevent illness than to spend it on treating people who are already ill. To what extent do you agree or disagree?'
    AND prompt_registry_key = 'ielts:writing:task2:web:prompt_registry:v1'
    AND prompt_record_ref = 'task2-web-template-v1'
    AND prompt_version = '2026-08-19.1'
    AND grading_pool = 'task2'
  ...
  IF v_activity_count <> 1
    ...
    RAISE EXCEPTION
      'Seed đề ngân sách y tế chưa đạt: activity=%, status=%, sections=%, lệch định nghĩa=%, scope active=%.',
      v_activity_count,
      v_activity_status,
      v_section_count,
