-- Dữ liệu nhận vào: activity đề phát triển kinh tế và môi trường ở trạng thái bản nháp, manifest
-- đúng checksum và hai class scope hiện hành của đề ngân sách y tế.
-- Việc chính: mở đúng CS.070626 và CS.160826, sao chép ngoại lệ roster đã duyệt,
-- rồi dựng lại roster bằng hàm production hiện hành.
-- Kết quả: hai lớp nhìn thấy đề mới với đúng danh sách học viên đang hoạt động.
-- Khi lỗi: transaction rollback toàn bộ; rollback vận hành bằng cách đóng scope/activity,
-- không xóa roster, phiên làm bài, nhận xét hoặc lịch sử.
BEGIN;
...
DECLARE
  ...
  v_source_empty_scope_count INTEGER;
  v_invalid_override_count INTEGER;
BEGIN
  SELECT count(*)
  INTO v_target_count
  FROM writing_practice.activity
  WHERE slug = 'writing-task2-economic-development-environment'
    AND content_version = '2026-09-13.1'
    AND manifest_checksum = '40f59639f512e633cb4214a311d07f6a4eff57180bb98e73ffb630bbcd118379'
  ...
  SELECT count(*)
  INTO v_source_empty_scope_count
  FROM writing_practice.activity_class_scope scope
  JOIN writing_practice.activity activity ON activity.id = scope.activity_id
  WHERE activity.slug = 'writing-task2-public-health-spending'
    AND scope.erp_course_class_id IN (1184, 1283)
  ...
  IF v_target_count <> 1
     ...
     OR v_source_empty_scope_count <> 0
     OR v_invalid_override_count <> 0 THEN
    RAISE EXCEPTION
      'Nguồn phát hành chưa đạt: target=%, template=%, source scopes=%, scope rỗng=%, override lỗi=%.',
      v_target_count,
      v_template_count,
      v_source_scope_count,
      v_source_empty_scope_count,
...
WITH target_scope AS (
  ...
  FROM writing_practice.activity_class_scope scope
  JOIN writing_practice.activity activity ON activity.id = scope.activity_id
  WHERE activity.slug = 'writing-task2-economic-development-environment'
    AND scope.erp_course_class_id IN (1184, 1283)
), source_override AS (
  SELECT
    scope.erp_course_class_id,
...
BEGIN
  ...
  WITH target_scope AS (
    ...
    SELECT scope.erp_course_class_id, scope.class_name_snapshot, DATE '2026-12-31' AS end_date
    FROM writing_practice.activity_class_scope scope
    JOIN writing_practice.activity activity ON activity.id = scope.activity_id
    WHERE activity.slug = 'writing-task2-public-health-spending'
      AND activity.status = 'active'
  ...
  IF v_activity_count <> 1
    ...
    RAISE EXCEPTION
      'Phát hành đề phát triển kinh tế và môi trường chưa đạt: activity=%, sections=%, definition diff=%, scopes=%, class diff=%, roster diff=%, scope rỗng=%, alias trùng=%, provisional=%.',
      v_activity_count,
      v_section_count,
