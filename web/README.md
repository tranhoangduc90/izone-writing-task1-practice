...
Bản demo `draft-inline-result-demo.html` dùng dữ liệu band 6.0 giả theo đúng response `essays` của Quick Aid để hiển thị từng thẻ chấm câu ngay trong handout. Demo cố ý bỏ qua hai trường `content` và `feedback`, tức màn tổng hợp TR/CC, đồng thời không gọi API, LMS, n8n hay database.
